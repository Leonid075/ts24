...
for _ in range(len(inds)//2):
    g_df.loc[len(g_df)] = scores.iloc[i.__next__()].tolist()[1:] + scores.iloc[i.__next__()].tolist()[1:]

#8.1
print(f"Оба предмета изучали {len(g_df)} учащихся")

#8.2
sns.displot(data=g_df, x="PG1", y="MG1", aspect=1,).figure.savefig("G1_compare")
sns.displot(data=g_df, x="PG2", y="MG2", aspect=1,).figure.savefig("G2_compare")
sns.displot(data=g_df, x="PG3", y="MG3", aspect=1,).figure.savefig("G3_compare")
